...
class TesseraTrasporti{
    ...
    get creditoResiduo(){return this.#credito;}
    ricarica(importo){
        if(importo <= 0)
            throw new TypeError();
...
class TesseraStudenti extends TesseraTrasporti{
    #istituto
    constructor(codice,credito,istituto) {
        super(codice,credito);
        if(typeof istituto !== "string" || istituto.length === 0)
            throw new TypeError();
        this.#istituto = istituto;
    }
    get istituto(){return this.#istituto}
    timbra(costo,linea){
        super.timbra(costo/2,linea);
    }
}
...
class Sensore{
    ...
    #modello
    #arr = []
    constructor(id,modello){
        if(!(/S-[0-9]{3}/.test(id)))
            throw new TypeError();
        if(typeof modello !== 'string' || modello.length === 0)
            throw new TypeError();
        this.#id = id
...
class SensoreSoglia extends Sensore{
    #sogliaMax
    constructor(id,modello,sogliaMax) {
        super(id,modello);
        if(typeof sogliaMax !== 'number')
            throw new TypeError();
        this.#sogliaMax = sogliaMax;
    }
    get sogliaMax(){return this.#sogliaMax}
    registra(valore){
        super.registra(valore);
        if(valore > this.#sogliaMax)
...
class Job{
    ...
    constructor(codice,priorita){
        if(!(/[A-Z]{2}[0-9]{3}/.test(codice)))
            throw new TypeError()
        if(!Number.isInteger(priorita) || priorita < 1 || priorita > 10)
            throw new TypeError()
        this.#codice = codice;
    ...
    get priorita(){return this.#priorita}
}
...
class GestoreProcessi{
    ...
    accoda(job){
        if(!(job instanceof Job))
            throw new TypeError()
        if(this.#coda.length === this.#maxSize)
    ...
    eseguiProssimo(){
        ...
        for(let i = 0; i < this.#coda.length; i++){
                if(this.#coda[i].priorita > max){
                    max = this.#coda[i].priorita
                    cod = this.#coda[i].codice
                    ind = i
                }
        }
        this.#coda.splice(ind,1)
        this.#eseguiti.push({codice: cod, priorita: max})
        return {codice: cod, priorita: max}
    }
    *storico(){
        let copia = [...this.#eseguiti].reverse()
        for(let i of copia){
            yield {
                codice: i.codice,
                priorita: i.priorita
...
assert.throws(() => gestore.accoda({ codice: "AB123", priorita: 5 }), TypeError, "Test 4 Fallito: Oggetto non istanza di Job non lancia TypeError");
